...
//    }
    ...
    public static function row_count_invoice($from_date, $to_date, $branch_id) {
        global $database;
        $where_condision = '';
        // $from_date = $database->escape_value($from_date);
        // $to_date = $database->escape_value($to_date);
        if($from_date != NULL && $to_date != NULL){
            $where_condision = " AND DATE(invoice_date) BETWEEN '$from_date' AND '$to_date' ";
        }
                
        $res = $database->doQuery("SELECT COUNT(*) FROM " . static::$table_name. " WHERE invoice_branch = '$branch_id'" .$where_condision. "");
        $count = $res[0][0];
        
        // echo '<pre>';
        // print_r("SELECT COUNT(*) FROM " . static::$table_name. " WHERE invoice_branch = '$branch_id'" .$where_condision. "");
        // die;
        return $count;
    }

    // public static function find_all_by_limit_offset_invoice($limit, $offset, $from_date, $to_date,$invoice_number) {
    //     global $database;
    //     $date_condition = '';
    //     $invoice_number_condition = '';
    //     $from_date = $database->escape_value($from_date);
    //     $to_date = $database->escape_value($to_date);
    //     $invoice_number = $database->escape_value($invoice_number);
        

    //     if($from_date != NULL && $to_date != NULL){
    //       $where_condision = " WHERE DATE(invoice_date) BETWEEN '$from_date' AND '$to_date'";
    //     }
    //     if($invoice_number != NULL){
    //         $where_condision = " WHERE invoice_number = '$invoice_number' ";
    //     }
    //     $object_array = self::find_by_sql("SELECT * FROM " . static::$table_name . $where_condision ." ORDER BY invoice_date DESC LIMIT $limit OFFSET $offset");
    //     return $object_array;
    // }

    public static function find_all_by_limit_offset_invoice($limit, $offset, $from_date, $to_date,$invoice_number, $branch_id) {
        ...
        if($invoice_number != NULL){
            $where_condision = " AND invoice_number = '$invoice_number' ";
        }
        $object_array = self::find_by_sql("SELECT * FROM " . static::$table_name. " WHERE  invoice_branch = '$branch_id' " .$where_condision. " ORDER BY DATE(invoice_date) DESC LIMIT $limit OFFSET $offset");


        // echo '<pre>';
        // print_r("SELECT * FROM " . static::$table_name . $where_condision ." AND  invoice_branch = '$branch_id' ORDER BY DATE(invoice_date) DESC LIMIT $limit OFFSET $offset");
        // die;
