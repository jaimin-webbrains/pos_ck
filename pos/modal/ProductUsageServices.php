...
class ProductUsageServices extends DatabaseObject{
    protected static $table_name="product_usage_services";
    ...
    public static function check_name($value){
        global $database;
        $value=$database->escape_value($value);
        $object_array=self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE volume LIKE '$value%' ");
        return $object_array;
    }
