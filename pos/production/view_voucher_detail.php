...
$get_voucher=mysqli_query($con,"select * from voucher where voucher_status=0");
...
while($voucher_data_array=mysqli_fetch_array($get_voucher))
{
	...
	if($expiry_date >= $current_date)
	{
		...
		array_push($rows, array("voucher_number" => $voucher_data_array['voucher_number'], "voucher_value" => $voucher_data_array['voucher_value'], "voucher_value_type" => $voucher_data_array['voucher_value_type']));
	}
}
