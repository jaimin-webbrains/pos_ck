  ...
  if(isset($_POST['color'])){
    ...
    if(count($checkCodeExists) > 0){
      ...
      if($_POST['color3']  == '') {
        ...
        {
          if($_POST['color2'] == $_POST['color3'])
          {
            $_SESSION["error"] = "Error..! Same code For the second and third color code detected";
            Functions::redirect_to("../invoice.php?invoice_id=".$invoice_id);
          }
          else{
            foreach($checkCodeExists3 as $rows);
            $row_data->color3 =$rows->code;
            if($_POST['percentage3'] == '')
            {
              $_SESSION["error"] = "Error..! Percentage for the third entry must not be empty";
              Functions::redirect_to("../invoice.php?invoice_id=".$invoice_id);
            }
            else{ $row_data->percentage3 = $_POST['percentage3']; }
          }
        }
        else{
          $_SESSION["error"] = "Error..! Code for the Third color does not exist.";
          Functions::redirect_to("../invoice.php?invoice_id=".$invoice_id);
        }
      }
      ...
      if($_POST['color4']  == '') {
        $row_data->color4 = 0;
        $row_data->percentage4 = 0;
      }
      else{
        $checkCodeExists4 = ColourTube::capture_code($_POST['color4']);
        if(count($checkCodeExists4) > 0)
        {
          if($_POST['color3'] == $_POST['color4'])
          {
            $_SESSION["error"] = "Error..! Same code For the third and fourth color code detected";
            Functions::redirect_to("../invoice.php?invoice_id=".$invoice_id);
          }
          else{
            foreach($checkCodeExists4 as $rows);
            $row_data->color4 =$rows->code;
            if($_POST['percentage4'] == '')
            {
              $_SESSION["error"] = "Error..! Percentage for the fourth entry must not be empty";
              Functions::redirect_to("../invoice.php?invoice_id=".$invoice_id);
            }
            else{ $row_data->percentage4 = $_POST['percentage4']; }
          }
        }
        else{
          $_SESSION["error"] = "Error..! Code for the Fourth color does not exist.";
          Functions::redirect_to("../invoice.php?invoice_id=".$invoice_id);
        }
      }
      ...
      if($_POST['color5'] == '') {
        $row_data->color5 = 0;
        $row_data->percentage5 = 0;
      }
      else{
        $checkCodeExists5 = ColourTube::capture_code($_POST['color5']);
        if(count($checkCodeExists5) > 0)
        {
          if(($_POST['color'] == $_POST['color2']) || ($_POST['color'] == $_POST['color3']) || ($_POST['color'] == $_POST['color4']) || ($_POST['color'] == $_POST['color5']) || ($_POST['color2'] == $_POST['color3']) || ($_POST['color2'] == $_POST['color4']) || ($_POST['color2'] == $_POST['color5']) || ($_POST['color3'] == $_POST['color4']) || 
            ($_POST['color3'] == $_POST['color5']) || ($_POST['color4'] == $_POST['color5']))
          {
            $_SESSION["error"] = "Error..! Either fourth or fifth same colour code detected! Please try again!";
            Functions::redirect_to("../invoice.php?invoice_id=".$invoice_id);
          }
          else{
            if($_POST['percentage5'] == '')
            {
              $_SESSION["error"] = "Error..! Percentage for the fifth entry must not be empty";
              Functions::redirect_to("../invoice.php?invoice_id=".$invoice_id);
            }
            else{
              foreach($checkCodeExists5 as $rowss);
              $row_data->color5 =$rowss->code;
              $row_data->percentage5 = $_POST['percentage5'];
            }
          }
        }

        else{
          $_SESSION["error"] = "Error..! Colour code does not exist for the Fifth Colour that you entered";
          Functions::redirect_to("../invoice.php?invoice_id=".$invoice_id);
        }
      }
      //////////////////////////////////
