      ...
      <div class="row" style="padding-top:30px;">
        ...
        <table id="tblData" class="table table-bordered" style="font-size:10px;width:100%;">
          ...
          <tbody>
            ...
            <?php
          // cklim check what is post from branch_employee.php to branch_employee_print.php ...always post "1" even select "all"
          // technical problem posting received showed "select all" but post as "1" admin. 
          // if($service_user == "all"){
...
$last_serviceid =0;   // check head count for product
$loopcount = 0;
  foreach($sub_invoice_obj as $calculatoon)
  {
 // cklim start of product commission and head count part 1
//   if ($last_serviceid !== $pro_data->id){
//   $last_serviceid = $pro_data->id;
          if($calculatoon->ops1_user == $service_data->id)
  ...
  //cklim product headcount 
  if ($last_serviceid != $pro_data->id){
                          $head_pro_count=$head_pro_count+$commission_pro_per;
                           if ( ($calculatoon->ops1 > 1))  {
                             $commission_pro_per1=1; } }


echo "ops1 = ";
echo $calculatoon->ops1;
echo ".....ops 2 =";
echo $calculatoon->ops2;
$last_serviceid = $pro_data->id; 
echo " ......  head count ops 1 and ops 2 =";
echo $commission_pro_per1;
echo "...... ";
echo $commission_pro_per;
echo " ....end of head count";
}
...
//cklim calculate for product com and head count end here cklim
//echo " i am at the last section  <br/>";
  // } check for duplicate items in single invoice 
}
...
foreach ($pro_object as $pro_data) {
  ...
  foreach($sub_invoice_obj as $calculatoon){
    ...
    if($calculatoon->ops1_user == $service_data->id){
      ...
      // cklim product head count second part....
      $head_pro_count=$head_pro_count+$commission_pro_per;
      if ( ($calculatoon->ops1>0 &&  $calculatoon->ops2==0))  {
        $commission_pro_per1=1;
        echo "another product head count ";
      }else if ($calculatoon->ops1>0 && $calculatoon->ops2>0) {
        $commission_pro_per1=0.5;
      }
      /*if ($commission_per1>0 && $commission_per1<1) {
