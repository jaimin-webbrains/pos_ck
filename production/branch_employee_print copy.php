...
// cklim start here for all users 
              ...
              foreach ( $user_object as $service_data) {
                ...
                foreach ($sub_inv as $calculatoon_1) {
                  ...
                  $InvoiceVoucher = Invoice::find_by_id($calculatoon_1->invoice_id);
                  $voucher = Voucher::find_by_voucher_number($InvoiceVoucher->invoice_voucher);
                  if(!empty($voucher)){
                  ...
                  // echo '<pre>';
                  // print_r($total_voucher_query);
                  while($voucher_fetch=mysqli_fetch_array($total_voucher_query))
                  {
                    $total_voucher_value_default=$voucher_fetch['voucher_value']+$total_voucher_value_default;
                    echo '<pre>';
...
if(!empty($voucher)){
  if($voucher->voucher_value_type == 0){
    $total_voucherr = $total_sum * $total_voucher_value_default/100;
  }else{
    $total_voucherr = $total_voucher_value_default;
  }
}

// $total_voucher_value = $total_sum - $total_voucherr;
echo "<td style='text-align:center;'> ".number_format($total_voucherr,2)." </td>";
...
else
{
  ...
  foreach ($sub_inv as $calculatoon_1) {
    ...
    $InvoiceVoucher2 = Invoice::find_by_id($calculatoon_1->invoice_id);
    $voucher2 = Voucher::find_by_voucher_number($InvoiceVoucher2->invoice_voucher);
    if(!empty($voucher)){
      if($voucher2->voucher_value_type == 0){
        $sym = '%';
      }else{
        $sym = '';
      }
...
if(!empty($voucher2)){
  if($voucher2->voucher_value_type == 0){
    $total_voucherr2 = $total_sum * $total_voucher_value_default/100;
  }else{
    $total_voucherr2 = $total_voucher_value_default;
  }
}
echo "<td style='text-align:center;'> ".number_format($total_voucherr2,2)." </td>";
