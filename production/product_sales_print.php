  ...
  <div class="container-fluid">
    ...
    ?>
      <div class="row" style="padding-top:30px;">
        <!-- table starts -->
        <?php
        $cat_object = ProductCode::find_all();
        ...
        <table id="tblData" class="table table-bordered" style="font-size:10px;width:100%;">
          <thead>
            ...
            <tr>
              ...
              <th colspan="<?php echo ($dateDiff+1); ?>" style="text-align:center;">Product Sales Report</th>
            </tr>
            <tr>
              <th>S/No</th>
              <th>Product Code</th>
            ...
            if(isset($from_date_val) && isset($to_date_val) ) {
              ...
              foreach(ProductCode::find_all() as $cat_data){
                ...
                while ($date1_ts <= $date2_ts) {
                  $count = 0;
                  if($branch_type == 0)
                    $item_data = InvoiceSub::find_by_type_code(1, $cat_data->code, date('Y-m-d', $date1_ts), $_POST['branch']);
                  else
                    $item_data = InvoiceSub::find_by_type_code_without_branch(1, $cat_data->code, date('Y-m-d', $date1_ts));
        ...
        <table id="tblData" class="table table-bordered" style="font-size:10px;width:100%;">
          <thead>
            ...
            <th><?php 
              ...
              ?></th>
              <th><?php if($branch_type == 0) echo $branch_obj->code; ?></th>
              <th colspan="13" style="text-align:center;">Product Sales Report</th>
            </tr>
            <tr>
              <th>S/No</th>
              <th>Product Code</th>
            ...
            if(isset($_POST['year'])) {
              ...
              foreach(ProductCode::find_all() as $cat_data){
                ...
                for($i=0; $i<12; $i++) {
                  ...
                  if($branch_type == 0)
                    $item_data = InvoiceSub::find_by_type_branch_code_type_date_range(1, $cat_data->code, $_POST['branch'], $strdate1, $strdate2);
                  else 
                    $item_data = InvoiceSub::find_by_type_branch_code_type_date_range_without_branch(1, $cat_data->code, $strdate1, $strdate2);
                  foreach($item_data as $live_count) {
                    $count = $count + $live_count->qty;
                    // $countt = $countt + ($live_count->qty * $live_count->unit_price);
                  }
