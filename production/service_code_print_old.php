        ...
        <table id="tblData" class="table table-bordered" style="font-size:10px;width:100%;">
          <thead>
            ...
            <tr>
              ...
              while ($date1_ts <= $date2_ts) {
                ...
                foreach(ProductUnit::find_all() as $cat_data){
                  $product_item2[$cat_data->p_code][] = 0;
                  $product_item4[$cat_data->p_code][] = 0;
                }
              }
              echo "<th> S </th>";
              echo "<th> R </th>";
          ...
          <tbody>
            ...
            if(isset($from_date_val) && isset($to_date_val) ) {
              ...
              foreach(ProductUnit::find_all() as $product_code) {
                $total_item2 = 0;
                $total_item4 = 0;

                echo "<tr>";
                echo "<td></td>";
                echo "<td>". $product_code->p_code . "</td>";
                ...
                $day_index = 0;
                while ($date1_ts <= $date2_ts) {
                  $total_item2 += $product_item2[$product_code->p_code][$day_index];
                  $total_item4 += $product_item4[$product_code->p_code][$day_index];

                  echo "<td>" . $product_item2[$product_code->p_code][$day_index] . "</td>";
                  echo "<td>" . $product_item4[$product_code->p_code][$day_index] . "</td>";
                ...
                $product_units = ProductUnit::check_code($product_code->p_code);
