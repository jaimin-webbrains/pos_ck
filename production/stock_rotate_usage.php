  ...
  <div class="">
    <div class="page-title">
      <div class="title_left">
        <h3 style="font-weight:800;">Stock Rotate Usage</h3>
      </div>
    ...
    <div class="row">
      <div class="col-md-6 col-sm-6 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            <h2 id="title" style="font-weight:700;"><?php echo (empty($stock_rotate_usage->id)) ? 'Add' : 'Edit'; ?> Stock Rotate Usage</h2>
            <div class="clearfix"></div>
          </div>
          <div class="x_content">
            <form id="formRole" action="proccess/stock_rotate_usage_process.php" method="post" class="form-horizontal form-label-left">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <input type="hidden" class="form-control" id="txtId" name="id" value="<?php echo $stock_rotate_usage->id; ?>" />
                  ...
                  <select class="form-control" name="branch" id="stock_branch">
                    <?php				  
                      foreach (Branch::find_all() as $branch) {
                        if($stock_rotate_usage->branch_id == $branch->id)
                ...
                <div class="form-group">
                  <label>Product</label>
                  <input type="text" class="form-control" placeholder="Product code ..." id="stock_code" name="code" value="<?php echo $stock_rotate_usage->p_code; ?>" onkeyup="checkProductCode(this.value)" required="">
                  <span id="txtHint_code"></span>
                </div>                

                <div class="form-group">
                  <label>Reasons</label>
                  <input type="text" class="form-control" placeholder="Reasons ..." id="stock_reasons" name="reasons" value="<?php echo $stock_rotate_usage->reasons; ?>" onkeyup="checkReasons(this.value)" required="">
                </div>

                <div class="form-group">
                  <label>Datetime</label>
                  <input type="datetime-local" class="form-control" id="stock_datetime" name="datetime" value="<?php echo $stock_rotate_usage->datetime == '' ? '' : date('Y-m-d\TH:i', strtotime($stock_rotate_usage->datetime)); ?>" required="">
                </div>

                <div class="form-group">
                  <label>Quantity</label>
                  <input type="text" class="form-control" placeholder="quantity ..." id="stock_quantity" name="quantity" value="<?php echo @$stock_rotate_usage->quantity; ?>"  required="">
                </div>
                
                <div class="modal-footer col-md-12 col-sm-12 col-xs-12">
                  <?php if(Functions::check_privilege_by_module_action("StockRotateUsage", "ins")) { ?>
                    <div class=" col-md-4 col-sm-4 col-xs-12">
                      <button id="btnSave" type="button" name="save" class="btn btn-block btn-success"><i class="fa fa-floppy-o"></i>&nbsp;Save</button>
                    </div>
                  <?php } ?>
                  <?php if(Functions::check_privilege_by_module_action("StockRotateUsage", "del")){ ?>
                    <div class=" col-md-4 col-sm-4 col-xs-12" style="display: <?php echo (empty($stock_rotate_usage->id)) ? 'none' : 'initial'; ?>">
                      <button id="btnDelete" type="button" name="delete" class="btn btn-block btn-danger" ><i class="fa fa-trash"></i>&nbsp;Delete</button>
                    </div>
                  <?php } ?>
                  <div class=" col-md-4 col-sm-4 col-xs-12">
                    <a href="stock_rotate_usage.php"><button type="button" name="reset" class="btn btn-block btn-primary"><i class="fa fa-history"></i>&nbsp;Reset</button></a>
                  </div>
                </div>
              </div>
            </form>
        ...
        <div class="col-md-12 col-sm-12 col-xs-12">
          <div class="x_panel">
            ...
            <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                ...
                <tbody>
                  <?php
                  $objects = StockRotateUsage::find_all();
                  $count = 0;
                  foreach ($objects as $obj) {
                    ...
                    <td>
                      <form action="stock_rotate_usage.php" method="post" style="float: left;">
...
$("#btnSave").click(function () {
  var id = <?php echo ($stock_rotate_usage->id) ? 1 : 0; ?>;
  if (id) {
