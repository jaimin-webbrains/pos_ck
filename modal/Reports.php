        ...
        foreach(ProductUsageServices::find_all() as $service_product){
          ...
          foreach($product_codes as $key=>$value){
            ...
            if($product_code == $p_code){
              array_push($code_arr,$service_product->p_code);
            }
          }
        }
        if(!empty($code_arr)){
          $result_arr = "'" . implode ( "', '", $code_arr ) . "'";
          for($month = 1; $month <= 12; $month++){
            $monthly_volume = 0;
            $object_array=InvoiceSub::find_by_sql("SELECT * FROM invoice_sub INNER JOIN invoice ON invoice_sub.invoice_id = invoice.id WHERE invoice_sub.item_type = '$item_type' AND invoice_sub.code IN ($result_arr) AND MONTH(invoice.invoice_date) = '$month' AND YEAR(invoice.invoice_date) = '$year' ".$branch_condition);
              
            if(count($object_array) > 0){
              $monthly_volume = 0;
              foreach($object_array as $obj){
                $object=ProductUsageServices::find_by_sql("SELECT * FROM product_usage_services where p_code LIKE '$obj->code'");
                $code = $object[0]->p_use_code;
                $volume =  explode(",",$object[0]->volume);
                $monthly_volume = $monthly_volume + floatval(trim($volume[array_search($p_code,explode(",",$code))])) * $obj->qty;
              }
            }
            $data_array[$month] = $monthly_volume;
          }
          ...
          for($day = 1; $day <= $days_in_month; $day++){
            ...
            if(count($object_array) > 0){
              ...
              foreach($object_array as $obj){
                ...
                $code = $object[0]->p_use_code;
                $volume =  explode(",",$object[0]->volume);
                $daily_volume = $daily_volume +  floatval(trim($volume[array_search($p_code,explode(",",$code))])) * $obj->qty;
              }
            }
    ...
    public static function find_S_R_product_usage_by_year_day_wise($from_date, $to_date, $branch_id, $p_code, $item_type){
        ...
        if(!empty($code_arr)){
          ...
          foreach ($period as $key => $value) {
            ...
            $object_array=InvoiceSub::find_by_sql("SELECT invoice_sub.*, invoice.invoice_date FROM invoice_sub LEFT JOIN invoice ON invoice_sub.invoice_id = invoice.id WHERE invoice_sub.item_type = '$item_type' AND invoice_sub.code IN ($result_arr) AND DATE(invoice.invoice_date) BETWEEN '$from_date' AND '$to_date' ".$branch_condition);
            ...
            if(count($object_array) > 0){
              ...
              foreach($object_array as $row){
                ...
                if($date == $period_date){
                  $object=ProductUsageServices::find_by_sql("SELECT * FROM product_usage_services where p_code LIKE '$row->code'");
                  $code = $object[0]->p_use_code;
                  $volume =  explode(",",$object[0]->volume);
                  $daily_volume = $daily_volume +  floatval(trim($volume[array_search($p_code,explode(",",$code))])) * $row->qty;
                }
              }
            }
