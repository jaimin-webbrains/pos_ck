...
class Colour extends DatabaseObject{
    protected static $table_name="colour";
    ...
    public static function check_desc($value){
        global $database;
        $value=$database->escape_value($value);
        $object_array=self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE description LIKE '$value%' ");
        return $object_array;
    }

    public static function find_colour_by_id($value){
        global $database;
        $value=$database->escape_value($value);
